...
export default class Flow extends PureComponent {
  ...
  onClick = () => {
    if (this.choose.opMain === 0) {
      ...
      this.volume = this.volume * this.multi; // L
      // this.dose = this.factor['1'] * this.total;
      this.volume  = this.volume  / this.factor['3'][this.choose.opTank<=1?0:1];
    } else if (this.choose.opMain === 1 && this.volume < 0) return;
    
    const min1 = this.round(this.volume * 5, 2);
    const max1 = this.round(this.volume * 10, 2);
    this.reslut1 = `${min1} ${this.metrics}/H ~ ${max1} ${this.metrics}/H `;
    const min2 = this.round(this.volume * 10, 2);
    const max2 = this.round(this.volume * 20, 2);
    this.reslut2 = `${min2} ${this.metrics}/H ~ ${max2} ${this.metrics}/H `;
    this.forceUpdate();
  }
